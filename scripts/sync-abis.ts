...
const CONTRACTS: ContractSpec[] = [
  ...
  {
    artifact: "RelayTeleportAdapter",
    output: "relay-teleport-adapter",
    exportName: "RELAY_TELEPORT_ADAPTER_ABI",
    description:
      "RelayTeleportAdapter — XCM InitiateTeleport adapter targeting the relay chain (DOT/PAS).",
  },
  {
    artifact: "KaruraAdapter",
    output: "karura-adapter",
    exportName: "KARURA_ADAPTER_ABI",
    description:
      "KaruraAdapter — Karura DEX (Dex.swapWithExactSupply) via XCM Transact (para 2000).",
  },
  {
    artifact: "MoonbeamAdapter",
    output: "moonbeam-adapter",
    exportName: "MOONBEAM_ADAPTER_ABI",
    description:
      "MoonbeamAdapter — EVM call via XCM Transact on Moonbeam (para 2004, Ethereum pallet 4).",
  },
  {
    artifact: "InterlayAdapter",
...
function toTsLiteral(value: unknown, indent = 0): string {
  ...
  if (Array.isArray(value)) {
    ...
    const items = value
      .map((v) => `${innerPad}${toTsLiteral(v, indent + 2)},`)
      .join("\n");
    return `[\n${items}\n${pad}]`;
  }
  if (typeof value === "object") {
    ...
    const items = entries
      .map(([k, v]) => `${innerPad}${k}: ${toTsLiteral(v, indent + 2)},`)
      .join("\n");
    return `{\n${items}\n${pad}}`;
  }
...
function generateFileContent(
  ...
  const items = (abi as unknown[])
    .map((entry) => `  ${toTsLiteral(entry, 2)},`)
    .join("\n");
...
function generateVaultCrossChainContent(
  vaultAbi: unknown[],
  routerPath: string,
): string {
  const timestamp = new Date().toISOString();
  const filtered = (vaultAbi as Array<{ type: string; name?: string }>).filter(
    (entry) =>
      ...
      (entry.type === "function" &&
        ...
        VAULT_CROSS_CHAIN_FUNCTIONS.has(entry.name)),
  );
...
function main(): void {
  ...
  const routerPath = resolve(
    routerPathIdx !== -1 && args[routerPathIdx + 1]
      ? args[routerPathIdx + 1]
      : join(_scriptDir, "../../obi.router"),
  );
    ...
    if (checkMode) {
      if (!existsSync(outputPath)) {
        ...
        const stripTimestamp = (s: string) =>
          s.replace(/^\/\/ Synced: .+$/m, "// Synced: <ts>");
        if (stripTimestamp(existing) !== stripTimestamp(content)) {
          console.error(
            `  [DRIFT]   ${spec.output}.ts — differs from current Foundry artifact`,
          );
          driftCount++;
        } else {
  ...
  try {
    ...
    const vaultCrossChainContent = generateVaultCrossChainContent(
      vaultAbi,
      routerPath,
    );
    ...
    if (checkMode) {
      if (!existsSync(outputPath)) {
        ...
        const stripTimestamp = (s: string) =>
          s.replace(/^\/\/ Synced: .+$/m, "// Synced: <ts>");
        if (
          stripTimestamp(existing) !== stripTimestamp(vaultCrossChainContent)
        ) {
          console.error(
            "  [DRIFT]   vault-cross-chain.ts — differs from current Foundry artifact",
          );
          driftCount++;
        } else {
  ...
  if (checkMode) {
    if (driftCount > 0) {
      console.error(
        `sync-abis: ${driftCount} file(s) have drifted. Run "pnpm sync:abis" to fix.`,
      );
      process.exit(1);
    } else {
...
function updateAbisIndex(abisDir: string): void {
  ...
  const allSpecs = [
    ...CONTRACTS,
    { output: "vault-cross-chain", exportName: "VAULT_CROSS_CHAIN_ABI" },
  ];

  const lines = allSpecs
    .map((s) => `export { ${s.exportName} } from './${s.output}.js';`)
    .sort();
